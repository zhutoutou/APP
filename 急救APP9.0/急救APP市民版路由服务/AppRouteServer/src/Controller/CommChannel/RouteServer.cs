        ...
        private void CheckConnectedStatus_Thread(object e)
        {
            ...
            while (true)
            {
                ...
                try
                {
                    foreach (var item in OnlineClents)
                    {
                        if (DateTime.Now.Subtract(ScsServer.Clients[item.intClientID].LastReceivedMessageTime) > new TimeSpan(0, 0, SharkHandsTime * 2 + 5)
